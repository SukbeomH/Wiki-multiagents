...
class Entity(BaseModel):
    ...
    start: Optional[int] = Field(
        default=None,
        ge=0,
        description="문서 내 시작 위치 (문자 인덱스)"
    )
    end: Optional[int] = Field(
        default=None,
        ge=0,
        description="문서 내 끝 위치 (문자 인덱스)"
    )
    ...
    @field_validator('end')
    @classmethod
    def validate_end_position(cls, v: Optional[int], info) -> Optional[int]:
        """끝 위치 검증"""
        if v is not None and 'start' in info.data and info.data['start'] is not None:
            if v <= info.data['start']:
                raise ValueError("끝 위치는 시작 위치보다 커야 합니다.")
        return v
    ...
    def validate_entity_type(cls, v: str) -> str:
        ...
        if v.upper() not in allowed_types:
            raise ValueError(f"지원하지 않는 엔티티 타입: {v}. 지원 타입: {allowed_types}")
        return v.upper()
    ...
    def validate_entity_name(cls, v: str) -> str:
        """엔티티 이름 검증"""
        if not v.strip():
            raise ValueError("엔티티 이름은 빈 문자열일 수 없습니다.")
        return v.strip()
...
class Relation(BaseModel):
    ...
    def validate_source(cls, v: str) -> str:
        """출발 엔티티 ID 검증"""
        if not v.strip():
            raise ValueError("출발 엔티티 ID는 빈 문자열일 수 없습니다.")
        return v.strip()
    ...
    def validate_target(cls, v: str) -> str:
        """도착 엔티티 ID 검증"""
        if not v.strip():
            raise ValueError("도착 엔티티 ID는 빈 문자열일 수 없습니다.")
        return v.strip()
    ...
    def validate_predicate(cls, v: str) -> str:
        ...
        if v.upper() not in allowed_predicates:
            raise ValueError(f"지원하지 않는 관계 타입: {v}. 지원 타입: {allowed_predicates}")
        return v.upper()
    ...
    def validate_different_entities(cls, v: str, info) -> str:
        ...
        if 'source' in info.data and v == info.data['source']:
            raise ValueError("출발 엔티티와 도착 엔티티는 달라야 합니다.")
        return v
...
class ExtractorIn(BaseModel):
    ...
    def validate_extraction_mode(cls, v: str) -> str:
        """추출 모드 검증"""
        allowed_modes = {"comprehensive", "fast", "focused"}
        if v.lower() not in allowed_modes:
            raise ValueError(f"지원하지 않는 추출 모드: {v}. 지원 모드: {allowed_modes}")
        return v.lower()
    ...
    def validate_entity_types(cls, v: Optional[List[str]]) -> Optional[List[str]]:
        ...
        if v is not None:
            ...
            for entity_type in v:
                if entity_type.upper() not in allowed_types:
                    raise ValueError(f"지원하지 않는 엔티티 타입: {entity_type}. 지원 타입: {allowed_types}")
